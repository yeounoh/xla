...
XlaOp ConvertRandomBitsToUniformFloatingPoint(XlaOp bits, XlaOp minval,
  ...
  return builder->ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
    ...
    auto is_f8 =
        (value_type == F8E5M2 || value_type == F8E4M3) && bit_type == U8;
    ...
    if (is_f32_or_f64) {
      ...
      if (value_type == F8E5M2) {
        auto mantissa = bits & ScalaLike(bits, 0x3u);
        auto exponent = ScalarLike(bits, static_cast<uint_16>(15) << 2);
        auto u8_result = exponent | mantissa;
        result = BitcastConvertType(u8_result, F8E5M2);
      } else {
        auto mantissa = bits & ScalaLike(bits, 0x7u);
        auto exponent = ScalarLike(bits, static_cast<uint_16>(7) << 3);
        auto u8_result = exponent | mantissa;
        result = BitcastConvertType(u8_result, F8E4M3);
      }
      return result - ScalarLike(result, 1.0);
    } else {
